...
from pipeline.ocr.errors import DuplicateOcrProviderError, OcrProviderNotFoundError
from pipeline.ocr.provider import OcrProvider


ProviderConstructor = Callable[[], OcrProvider]
PluginRegistration = Callable[["OcrProviderFactory"], None]
...
class OcrProviderFactory:
    ...
    def discover_default_plugins(cls) -> "OcrProviderFactory":
        """Discover and register all modules in ``pipeline.ocr_plugins``."""
        factory = cls()
    ...
    def register(self, name: str, constructor: ProviderConstructor) -> None:
        ...
        if not name.strip():
            message = "An OCR provider name must not be empty."
            raise ValueError(message)
        if name in self._constructors:
            message = f"An OCR provider named {name!r} is already registered."
            raise DuplicateOcrProviderError(message)
...
def _register_plugin(plugin_module: ModuleType, factory: OcrProviderFactory) -> None:
    registration = getattr(plugin_module, "register_providers", None)
    if not callable(registration):
        message = f"OCR plugin {plugin_module.__name__!r} has no register_providers function."
        raise RuntimeError(message)
